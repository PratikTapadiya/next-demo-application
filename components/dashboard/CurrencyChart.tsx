...
function CustomTooltip({ active, payload, label, rawRates }: CustomTooltipProps) {
...
export default function CurrencyChart() {
  ...
  const [rawRates, setRawRates] = useState<Record<string, Record<string, number>>>({})
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      for (const quote of quotes) {
        const firstDate = dates.find((d) => response.rates[d]?.[quote] !== undefined)
      ...
      setError('Could not load exchange rate data. Check your connection and try again.')
    } finally {
      setLoading(false)
    }
  ...
  const availableToAdd = CURRENCY_CODES.filter((c) => c !== base && !quotes.includes(c))
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
      <h2 className="text-base font-semibold text-gray-900 mb-5">Currency Comparison</h2>

      {/* Controls */}
      <div className="flex flex-wrap items-center gap-4 mb-5">
        {/* Base selector */}
        <div className="flex items-center gap-2">
          <label className="text-sm text-gray-500 whitespace-nowrap">Base</label>
          <select
            value={base}
            onChange={(e) => setBase(e.target.value)}
            className="text-sm border border-gray-400 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-black"
          >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {quotes.length < 5 && (
            <select
              ...
              className="text-xs border border-dashed border-gray-300 rounded-full px-2 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-gray-500"
            >
      ...
      {!loading && !error && chartData.length > 0 && (
        <ResponsiveContainer width="100%" height={350}>
          <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            />
            <Tooltip
              content={<CustomTooltip rawRates={rawRates} />}
            />
            <Legend
